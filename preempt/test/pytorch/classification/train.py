...
def train_one_epoch(model, criterion, optimizer, data_loader, device, epoch, args, model_ema=None, scaler=None):
    ...
    for i, (image, target) in enumerate(metric_logger.log_every(data_loader, args.print_freq, header)):
        ...
        # metric_logger.meters["img/s"].update(batch_size / (time.perf_counter() - start_time))
...
def load_data(traindir, valdir, args):
    ...
    if args.cache_dataset and os.path.exists(cache_path):
        ...
        if args.cifar10:
            dataset_test = torchvision.datasets.CIFAR10('./data', train=False, download=True, transform=preprocessing)
        else:
...
def main(args):
    # just for training test, no need to save model
    # if args.output_dir:
